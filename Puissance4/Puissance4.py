...
def reset():
    global fini, plateau, commence, player
    for i in r_col:
        for j in r_lig:
            plateau[i][j] = 0
    fini = False
    commence = autreCouleur[commence]
    player = commence
...
def gagnant(verbose = False):
    ...
    score = count3[1]-count3[2]+count2[1]-count2[2]+places[1]-places[2]
    if score > 0:
        return (1, score)
    elif score < 0:
        return (2, -score)
...
#print("  "*depth + "  placed = %s - gagnant = (%s, %s)" % (c, g, l))
            ...
            if depth <= profondeur_recherche:
                (p, score) = trouveMeilleurePosition(joueur, autreCouleur[joueurCourant], depth+1)
                if score > bestMove[1]:
                    bestMove = [c, score]
...
def ordinateurJoue():
    ...
    if nbPionsPlaces() == 0:
        # Le meilleur coup a jouer lorsque l'on commence est la colonne du milieu
        c = 3
    elif nbPionsPlaces() == 1 and plateau[3][5] == 0:
        # Le meilleur coup a jouer lorsque l'on joue en second et que le premier n'a pas joue au milieu
        c = 3
    elif nbPionsPlaces() < 0:
        c = random.randint(1, 5)
        if c < 2 or c > 4:
            c = random.randint(0, 6)
        print("Place un pion au hasard (premiers coups) en %s" % c)
    else:
        placed = False
        choose = -1
...
#while fini != True:
    ...
    for i in r_col:
        for j in r_lig:
            pygame.draw.circle(ecran, couleurs[plateau[i][j]],  (h_remainder+(2*i+1)*(rayon+margin), v_remainder+(2*j+1)*(rayon+margin)), rayon)
    g = gagnant()
    if g[1] == 1000000:
        print("Joueur %s gagne !" % g[0])
        infos = police.render("Joueur %s gagne !" % g[0], True, couleurs[g[0]])
