...
while quitter != True:
    ...
    if y >= 0 and X < largeur:
        ...
        v_y -= trainee*V*V*v_y/V
        v_x -= trainee*V*V*v_x/V
        v_y = v_y + a_y
        x = x+v_x
        y = y+v_y
        pygame.draw.rect(ecran, rouge, (X, Y, 1, 1), 1)

        infos = police.render("V = %s, Vx = %s , Vy = %s, Ay = %s, alpha = %s, trainee = %s" % (V, v_x, v_y, a_y, alpha, trainee), True, rouge)
