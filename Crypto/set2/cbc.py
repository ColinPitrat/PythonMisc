...
def pad(content, length):
    while len(content) % length != 0:
        content += '\x04'
    return content
...
def cipherAES128ECB(content, key):
...
def cipherAES128CBC(content, key, iv):
    result = ""
    prev = iv
    content = pad(content, 16)
    for i in range(0, len(content)/16):
        chunk = content[i*16:(i+1)*16]
        xor_chunk = xorStr(chunk, prev)
        prev = cipherAES128ECB(xor_chunk, key)
        result += prev
    return result

def decipherAES128CBC(content, key, iv):
    result = ""
    prev = iv
    for i in range(0, len(content)/16):
        chunk = content[i*16:(i+1)*16]
        xor_chunk = decipherAES128ECB(chunk, key)
        result += xorStr(xor_chunk, prev)
        prev = chunk
    result = unpad(result)
    return result

# Challenge 2: Decipher this AES CBC encrypted message
def decipher_message():
    b64_ciphered = ("CRIwqt4+szDbqkNY+I0qbNXPg1XLaCM5etQ5Bt9DRFV/xIN2k8Go7jtArLIy"
    ...
    deciphered = decipherAES128CBC(ciphered, key, '\x00'*16)
